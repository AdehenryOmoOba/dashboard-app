...
export async function GET() {
  try {
    await sql.begin(async (tx) => {
      // Ensure a clean slate to match the expected schema
      await tx`DROP TABLE IF EXISTS invoices CASCADE`;
      await tx`DROP TABLE IF EXISTS customers CASCADE`;
      await tx`DROP TABLE IF EXISTS users CASCADE`;
      await tx`DROP TABLE IF EXISTS revenue CASCADE`;
